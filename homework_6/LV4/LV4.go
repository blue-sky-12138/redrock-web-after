...
func main(){
	...
	router.POST("/ChangePassword/Create", func(context *gin.Context) {
		...
		if tem.ChangePassword!=tem.ChangePasswordAgain{
			...
			prepare:=fmt.Sprintf("select MD5salt from users_information where telephone_number=%d",login.ID)
			...
			data.data=tem.ChangePassword
			data.Cryptography(salt)
			login.update(dataBase,"password",data.result)
			PastContent.Content="更改完成"
			context.Redirect(http.StatusMovedPermanently,"/Past")
		}
	})
...
func (user *User)checkPassword(database *sql.DB)bool{
	...
	prepare:=fmt.Sprintf("select password,MD5salt from users_information where name='%s'",user.name)
	stmt,err:=database.Query(prepare)
	CheckError(err)
	defer stmt.Close()
	var (
	...
	if data.Cryptography(salt);tem==data.result{
		return true
	}
	return false
}
...
//MD5加密
func CryptographyNow(data string)string{
	var dc dataCryptographyMD5
	dc.data=data
	dc.Cryptography(time.Now().Unix())
	return dc.result
}
