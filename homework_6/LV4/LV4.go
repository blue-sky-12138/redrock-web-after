...
type MessageHistory struct {	//留言墙结构体
	TimeDate []string
	Author []string
	Information []string
}
...
func init(){
	messageHistory.TimeDate=make([]string,10)
	messageHistory.Author=make([]string,10)
	messageHistory.Information=make([]string,10)
}
...
func main(){
	...
	//查看历史记录
	router.GET("/MessageHistory", func(context *gin.Context) {
		messageHistory.findHistory(dataBase)
		context.HTML(http.StatusOK,"MessageHistory.html",messageHistory)
	})

	//留言
	router.GET("/LeaveMessage", func(context *gin.Context) {
		context.HTML(http.StatusOK,"LeaveMessage.html",nil)
	})
	router.POST("/LeaveMessage/Create", func(context *gin.Context) {
		temMessage,_:=context.GetPostForm("LeaveMessage")
		login.leaveMessage(dataBase,temMessage)
...
func (messageHistory *MessageHistory)findHistory(database *sql.DB){
	//倒序查询前十条留言
	prepare:=fmt.Sprintf("select time,user,information from message_history order by id desc limit 10")
	stmt,err:=database.Query(prepare)
	defer stmt.Close()
	CheckError(err)
	var i=0
	for stmt.Next(){
		stmt.Scan(&messageHistory.TimeDate[i],&messageHistory.Author[i],&messageHistory.Information[i])
		i++
	}
}

//执行更改操作
func (user *User)leaveMessage(database *sql.DB,leaveMessage string){
	prepare:=fmt.Sprintf("insert into message_history (time,user,information)value('%d-%d-%d %d:%d:%d','%s','%s')",
		time.Now().Year(),time.Now().Month(),time.Now().Day(),time.Now().Hour(),time.Now().Minute(),time.Now().Second(),
		user.name,leaveMessage)
	stmt,err:=database.Prepare(prepare)
	defer stmt.Close()
	CheckError(err)
	stmt.Exec()
